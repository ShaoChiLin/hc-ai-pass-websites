...
async function main() {
  ...
  try {
    ...
    if (error.status === 409) {
      /*
       * 也不要叫人「回申請完成頁再點一次」。那顆按鈕的 href 是送件當下就組好的，
       * token 寫死在裡面——再點一次送的是同一串已經死掉的綁定碼，同樣回 409。
       * 聊天室綁定那條退路用的也是同一串，一樣沒救。真正有救的只有重送一份申請。
       *
       * 另外「已使用」要特別講：那代表綁定其實已經成功過一次，這時候叫人重送申請
       * 反而是多做一件事，還會多一筆重複案件。伺服器的訊息把三種原因併成一句，
       * 前端分不出來，所以兩種可能都講清楚，讓使用者自己對照。
       */
      render(
        '這個綁定碼不能用了',
        `${error.message}重新登入 LINE 沒有用，回上一頁再點一次綁定按鈕也沒有用——那顆按鈕帶的是同一串綁定碼。如果你剛才已經成功綁過這個案件，那就已經完成了，不必再綁一次；否則請重新送一份申請，取得新的綁定碼。`,
        closeButton(),
      );
      return;
    }
